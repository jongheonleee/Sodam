...
### 3. 시스템 아키텍처 설계
>   ![시스템 아키텍처](./docs/design/[Sodam🍃]시스템아키텍처.png)
...
### 1. Spring MVC의 성능 한계를 Kotlin Coroutine & Spring WebFlux 를 적용하여 성능 개선
> - Locust를 활용하여 기존 프로젝트(Spring MVC)의 성능을 테스트했습니다.
> - 성능 테스트 시나리오를 위해 작업 시간이 **5초정도 소요되는 API 기능을 구성했고 해당 API로 최대 1000명 사용자가 동시에 요청을 보내게 했습니다.**
> - 결과는 **특정 시점을 기준으로 RPS가 40정도에서 그치고 내부적으로 500 Internal Server Error 가 발생하는 것을 확인했습니다.**
> - **이는 Spring MVC은 Sync/Blocking 방식으로 구동되기 때문에 I/O 발생시 Thread 대기와 그에 따른 빈번한 Context Switching 발생하여 컴퓨터의 고부하가 발생되어 서버가 다운된 것을 알게되었습니다.**
> - **이 문제를 개선하고자 Spring WenFlux와 Kotlin Coroutine 기반의 Async/Non-Blocking 을 활용하게 되었습니다. 이를 통해 RPS가 110~120 정도까지 나오는 것을 확인했습니다.**
...
### 0. Spring MVC의 성능 한계를 Kotlin Coroutine & Spring WebFlux 를 적용하여 성능 개선
> - 현재 문제는 **회원 서비스 오브젝트에 비즈니스 로직과 암호화 로직이 혼재**되어 있습니다.
> - 또한, 해당 기술 로직은 여러 부분에 나타나고 있습니다.
> - 이는 서비스 오브젝트에 작성된 비즈니스 로직에 대한 가독성을 저해시킬 뿐만 아니라, 변경에도 유연하게 대처할 수 없습니다.
> - **이 문제를 해결하고자 AOP와 애너테이션을 활용하여 비즈니스 로직과 기술적 로직을 분리했습니다.**
