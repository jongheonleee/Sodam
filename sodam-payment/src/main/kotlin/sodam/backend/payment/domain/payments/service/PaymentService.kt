...
class PaymentService(
    ...
    @Transactional
    suspend fun capture(order: OrdersEntity): Boolean {
        ...
        return try {
            ...
            order.pgStatus = when {
                ...
                e is WebClientResponseException -> { // 핵심 부분
                    // { code, message } -> UTF8로 인코딩 되어 있음, 따라서 String으로 바로 읽으면 문자 깨짐
                    // ByteArray로 가져온 것을 UTF8 형식으로 전환해야함 (기본값 UTF8로 구성되어 있음)
                    val json = String(e.responseBodyAsByteArray)
                    val errorResponse = objectMapper.readValue(json, TossPayApiError::class.java)
            ...
            // 이 과정에서 오류가 발생한다면, 지연 -> recapture 처리
