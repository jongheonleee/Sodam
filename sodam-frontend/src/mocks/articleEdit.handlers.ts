...
export const articleEditHandlers = [
    // 수정 게시글 조회 요청 처리
    http.get('/api/articles/edit/1', ({request, params}) => {
        return HttpResponse.json({
            code : 200,
            result : 'SUCCESS',
            message : '수정 게시글 조회 성공',
            data : {
                id : 1,
                title : '테스트용 수정 게시글 제목',
                category : {

                },
                summary : '테스트용 수정 게시글 요약',
                content : '테스트용 수정 게시글 내용',
                images: [{
    ...
    // 게시글 수정 요청 처리
    http.put('/api/articles/1', ({request, params}) => {
        return HttpResponse.json({
            code : 200,
            result : 'SUCCESS',
            message : '게시글 수정 성공',
